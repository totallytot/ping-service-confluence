AJS.$(document).ready(function () {
    ...
    AJS.$(document).on('change', '#monitoredSpaceKeys', function(e) {
        if (e.added) {
            ...
            if (dataObject.keysToDel.has(e.added.id)) dataObject.keysToDel.delete(e.added.id);
            else dataObject.keysToAdd.add(e.added.id);
        }
        if (e.removed) {
            if (dataObject.keysToAdd.has(e.removed.id)) dataObject.keysToAdd.delete(e.removed.id);
            else dataObject.keysToDel.add(e.removed.id);
    ...
    AJS.$(document).on('change', '#affectedGroups', function(e) {
        if (e.added) {
            console.log('add ' + e.added.id);
            if (dataObject.groupsToDel.has(e.added.id)) dataObject.groupsToDel.delete(e.added.id);
            else dataObject.groupsToAdd.add(e.added.id);

        }
        if (e.removed) {
            if (dataObject.groupsToAdd.has(e.removed.id)) dataObject.groupsToAdd.delete(e.removed.id);
            else dataObject.groupsToDel.add(e.removed.id);
            console.log('remove ' + e.removed.id);
        }
    });
    /*
